...
Since we are asked to compare between the 2 arms (control and experimental), I applied facet_wrap to better visualize it. The control arm have overall lower values throughout the 8 weeks as compared to the experimental arm which has an overall increasing trend. The control arm also has much more variability and fluctuations throughout the 8 weeks with no apparent trend.
...
The graph demonstrates a positive association  -- as true mu values increase, the proportion of times the null is rejected (power) also increase. This is because a larger effect size is associated with increased statistical power, making it easier to detect a statistically significant difference.
...
When the true mean is less than 4, the rejected sample mean mus are different compared to the true means because effect size and power is small. However, when the mean is 4 or larger, the rejected sample mean mus are about the same as the true mu because both effect size and power are larger.
